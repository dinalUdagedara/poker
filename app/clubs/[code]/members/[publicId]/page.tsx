...
import { memberChips } from '@/lib/server/counter'
...
export default async function MemberPage({ params }: PageProps<'/clubs/[code]/members/[publicId]'>) {
  ...
  const [club, member, chips] = await Promise.all([
    clubForMember(user, code),
    memberDetail(user, code, publicId),
    memberChips(user, code, publicId),
  ]).catch(orElse(code))

  return <MemberDetailPanel club={club} member={member} chips={chips} />
}
